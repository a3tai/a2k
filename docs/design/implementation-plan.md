...
3. **Setup, contexts, connectors**: `a3t setup` resolving org -> team -> project -> principal defaults into a reviewable bootstrap plan; connector catalog entries (`category: connector` manifests with org/context configSchema levels and credential bindings) materialized by the per-agent writers; context resolution per `(principal, project)` with directory-first selection, so concurrent per-project setups need no mode switch; approval writes agent configs, skills, connectors, and MCP client settings.
